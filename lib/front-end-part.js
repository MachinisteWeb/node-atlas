 ...
 */
exports.prepareRenderLanguage = function (variations, routeParameters, request, response, viewsPath, currentPath, path) {
    var NA = this;
    ...
    NA.prepareRenderPath(variations, routeParameters, request, response, viewsPath, currentPath, path);
};
 ...
 * @param {string} path                   Url path for this render.
 */
exports.prepareRenderPath = function (variations, routeParameters, request, response, viewsPath, currentPath, path) {
    var NA = this,
     ...
     */
    variations.urlRootPath = NA.webconfig.urlRoot;
     ...
     * 
     */
    variations.urlSubPath = NA.webconfig.urlRelativeSubPath;
     ...
     * https://www.example.here
     */
    variations.urlBasePath = NA.webconfig.urlRoot + NA.webconfig.urlRelativeSubPath;
     ...
     * /example/this/
     */
    variations.urlFilePath = currentPath;
     ...
     * ?title=Haeresis
     */
    variations.urlQueryPath = query && query[1] ? "?" + query[1] : "";
     ...
     */
    variations.urlPath = variations.urlBasePath + currentPath + variations.urlQueryPath;
    if (request) {
        variations.urlPath = "http" + ((NA.webconfig.httpSecure) ? "s" : "") + '://' + request.get("host") + request.originalUrl;
    }

    /* Next preparation render for variation. */
    NA.prepareRenderVariation(variations, routeParameters, request, response, viewsPath, currentPath, path);
};
 ...
 * @param {string} path                   Url path for this render.
 */
exports.prepareRenderVariation = function (variations, routeParameters, request, response, viewsPath, currentPath, path) {
    var NA = this,
    ...
    if (request) {
         ...
         * { "selector": "test" }
         */
        variations.params = request.params;
    }
     ...
     * @memberOf NA#webconfig
     */
    variations.common = NA.openVariation(NA.webconfig.commonVariation, variations.languageCode);
    if (variations.languageCode) {
     ...
     * @memberOf NA#routeParameters
     */
    variations.specific = NA.openVariation(routeParameters.variation, variations.languageCode);
    if (variations.languageCode) {
         ...
         * @memberOf NA#variations
         */
        variations.specific = extend(true, NA.openVariation(routeParameters.variation, undefined, true), variations.specific);
    }

    /* Nexts Step for render. */
    NA.prepareRenderParameters(variations, routeParameters, request, response, viewsPath, currentPath, path);
};
 ...
 * @param {string} path                   Url path for this render.
 */
exports.prepareRenderParameters = function (variations, routeParameters, request, response, viewsPath, currentPath, path) {
    var NA = this;
     ...
     * @memberOf NA#variations
     */
    variations.routeParameters = routeParameters;
     ...
     * @memberOf NA#variations
     */
    if (variations.routeParameters.url) {
        variations.routePath = path;
    }
     ...
     * @memberOf NA#variations
     */
    variations.webconfig = NA.webconfig;

    /* Nexts Step for render. */
    NA.changeVariationsCommon(variations, routeParameters, request, response, viewsPath, currentPath, path);
};
 ...
 * @param {string} currentPath            Url from `url` value for this render.
 */
exports.changeVariationsCommon = function (variations, routeParameters, request, response, viewsPath, currentPath) {
    var NA = this;
    ...
    if (typeof NA.controllers[NA.webconfig.commonController] !== 'undefined' &&
        typeof NA.controllers[NA.webconfig.commonController].changeVariations !== 'undefined') {
             ...
             */
            NA.controllers[NA.webconfig.commonController].changeVariations.call(NA, { variations: variations, request: request, response: response },

            /**
             * Next steps after changeVariations is done.
             * @callback changeVariations~callback
             * @param {Object} [variations] Variation object with new values.
             */
            function (tempVariation) {
                variations = (tempVariation) ? tempVariation : variations;
                NA.changeVariationsSpecific(variations, routeParameters, request, response, viewsPath, currentPath);
            });
    /* ...else, just continue. */
    } else {
        NA.changeVariationsSpecific(variations, routeParameters, request, response, viewsPath, currentPath);
 ...
 * @param {string} currentPath            Url from `url` value for this render.
 */
exports.changeVariationsSpecific = function (variations, routeParameters, request, response, viewsPath, currentPath) {
    var NA = this;
    ...
    if (typeof NA.controllers[routeParameters.controller] !== 'undefined' &&
        typeof NA.controllers[routeParameters.controller].changeVariations !== 'undefined') {
            ...
            NA.controllers[routeParameters.controller].changeVariations.call(NA, { variations: variations, request: request, response: response }, function (tempVariation) {
                variations = (tempVariation) ? tempVariation : variations;
                NA.changeDomCommon(variations, routeParameters, request, response, viewsPath, currentPath);
            });
    } else {
        /* ...else, just continue. */
        NA.changeDomCommon(variations, routeParameters, request, response, viewsPath, currentPath);
 ...
 * @param {string} currentPath            Url from `url` value for this render.
 */
exports.changeDomCommon = function (variations, routeParameters, request, response, viewsPath, currentPath) {
    var NA = this,
        ejs = NA.modules.ejs,
        pug = NA.modules.pug,
    ...
    NA.openTemplate(routeParameters, viewsPath, function (data) {
         ...
         * @memberOf NA#variations
         */
        variations.filename = pathM.join(NA.serverPath, NA.webconfig.viewsRelativePath, routeParameters.view);

        try {
            /* Transform ejs/pug data and inject incduded file. */
            data = engine.render(data, variations);
        } catch (err) {
        ...
        if (typeof NA.controllers[NA.webconfig.commonController] !== 'undefined' &&
            typeof NA.controllers[NA.webconfig.commonController].changeDom !== 'undefined') {
                 ...
                 */
                NA.controllers[NA.webconfig.commonController].changeDom.call(NA, { dom: data, variations: variations, request: request, response: response },
                 ...
                 */
                function (data) {
                    NA.changeDomSpecific(data, variations, routeParameters, request, response, currentPath);
                });
        /* ...else, just continue. */
        } else {
            NA.changeDomSpecific(data, variations, routeParameters, request, response, currentPath);
 ...
 * @param {string} currentPath            Url from `url` value for this render.
 */
exports.changeDomSpecific = function (data, variations, routeParameters, request, response, currentPath) {
    var NA = this;

    if (typeof NA.controllers[routeParameters.controller] !== 'undefined' &&
        typeof NA.controllers[routeParameters.controller].changeDom !== 'undefined') {
            /** Use the `NA.controllers[<controller>].changeVariations(...)` function if set... */
            NA.controllers[routeParameters.controller].changeDom.call(NA, { dom: data, variations: variations, request: request, response: response }, function (data) {
                NA.intoBrowserAndFiles(data, variations, routeParameters, request, response, currentPath);
            });
    } else {
        /** ...else, just continue. */
        NA.intoBrowserAndFiles(data, variations, routeParameters, request, response, currentPath);
 ...
 * @param {string} currentPath            Url from `url` value for this render.
 */
exports.intoBrowserAndFiles = function (data, variations, routeParameters, request, response, currentPath) {
    var NA = this;

    /* Inject CSS into DOM... */
    if (NA.webconfig.injectCss || routeParameters.injectCss) {
        NA.injectCss(data, routeParameters.injectCss, function (data) {
            NA.renderTemplate(data, variations, routeParameters, request, response, currentPath);
        });
    /* ...or do nothing. */
    } else {
        NA.renderTemplate(data, variations, routeParameters, request, response, currentPath);
 ...
 * @param {string} currentPath            Url from `url` value for this render.
 */
exports.renderTemplate = function (data, variations, routeParameters, request, response, currentPath) {
    var NA = this,
    ...
    if (typeof response !== 'undefined') {
        ...
        async.parallel([
            ...
            NA.response(request, response, data, routeParameters, variations);
        });
    }
};
...
exports.render = function (path, options, request, response) {
    ...
    NA.loadController(
        ...
        function () {
            ...
            NA.prepareRenderLanguage(variations, routeParameters, request, response, viewsPath, currentPath, path);
        }
    );
};