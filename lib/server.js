...
exports.initMiddlewares = function () {
    var NA = this,
         ...
         * @default false
         */
        forceDomain = NA.webconfig.forceDomain,
    ...
    /* Execute express middleware for all routes. */
    getMiddlewares = (
        ...
        require(path.join(NA.serverPath, NA.webconfig.middlewaresRelativePath, NA.webconfig.middlewares)).bind(NA)
    : function() { return []; };

    for (i = 0, middlewares = getMiddlewares(), l = middlewares.length; i < l; i++) {
...
exports.initLessProcess = function () {
    ...
    if (NA.webconfig.less && !NA.webconfig.cssBundlingBeforeResponse) {
        ...
        NA.express.use(lessMiddleware(path.join(NA.webconfig.assetsRelativePath), {
            ...
            render: {
                compress: NA.webconfig.less.compress || compressValue,
                sourceMap: NA.webconfig.less.sourceMap || sourceMapValue
            }
        }));
    }
};
...
exports.initStylusProcess = function () {
    ...
    if (NA.webconfig.stylus && !NA.webconfig.cssBundlingBeforeResponse) {
        stylusMiddleware = NA.modules.stylus.middleware({
            ...
            compress: NA.webconfig.stylus.compress || compressValue,
            sourcemap: NA.webconfig.stylus.sourceMap || sourceMapValue,
            force: NA.webconfig.stylus.force || forceValue,
            firebug: NA.webconfig.stylus.firebug || firebugValue,
            linenos: NA.webconfig.stylus.linenos || linenosValue
        });
...
function addFrontSockets(NA) {
    ...
    if (NA.webconfig.urlSocketsFile) {
        NA.express.get(url.format(path.join("/", NA.webconfig.urlRelativeSubPath, NA.webconfig.urlSocketsFile)), function (request, response) {
...
exports.initSockets = function (session, optionSession, next, fallback) {
     ...
     * @memberOf NA#
     */
    NA.io = socketio(server, optionIo);
