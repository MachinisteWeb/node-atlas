...
exports.atlasConfigurations = function (session, optionSession) {
    ...
    NA.httpServer.get(NA.webconfig.urlRelativeSubPath + "/node-atlas/socket.io.js", function (request, response) {
        response.setHeader("Content-type", "text/javascript; charset=utf-8");
        response.write(`(function () {
            var optionsSocket = ("${NA.webconfig.urlRelativeSubPath.slice(1)}" !== "") ? { 
                path: "/" + "${NA.webconfig.urlRelativeSubPath.slice(1)}" + ("${NA.webconfig.urlRelativeSubPath.slice(1)}" ? "/" : "") + "socket.io" 
            } : undefined;
            
            window.NA = window.NA || {};
            window.NA.io = io;
            window.NA.socket = NA.io.connect(("${NA.webconfig.urlRelativeSubPath.slice(1)}" !== "") ? "${NA.webconfig.urlRoot}" : undefined, optionsSocket);
        }());`);
        response.end();
    });
    ...
    /**
     * Define this function for configure all globals Sockets.
     * @function setSockets
     * @memberOf NA#websiteController[]
     */
    if (typeof NA.websiteController[NA.webconfig.commonController] !== 'undefined' &&
        typeof NA.websiteController[NA.webconfig.commonController].setSockets !== 'undefined') {
            NA.websiteController[NA.webconfig.commonController].setSockets.call(NA);
    }
    ...
    NA.forEach(NA.webconfig.routes, function (route) {
        ...
        if (route.controller) {
            ...
            /**
             * Define this function for configure all Sockets relative to a specific route.
             * @function changeSockets
             * @memberOf NA#websiteController[]
             */
            if (controller.changeSockets) {
                controller.changeSockets.call(NA);
            }
        }
    });
...
exports.response = function (request, response, data, currentRouteParameters, currentVariation) {
    var NA = this,
         ...
         * @default "utf-8"
         */
        charset = currentVariation.currentRouteParameters.charset || currentRouteParameters.charset || NA.webconfig.charset,
         ...
         * @default "text/html"
         */
        mimeType = currentVariation.currentRouteParameters.mimeType || currentRouteParameters.mimeType || NA.webconfig.mimeType,
         ...
         * @default "text/html"
         */
        headers = currentVariation.currentRouteParameters.headers || currentRouteParameters.headers || {};
...
exports.executeRequest = function (getSupport, postSupport, putSupport, deleteSupport, objectPath, options, path) {
    ...
    if (getSupport) {
        NA.httpServer.get(objectPath, function (request, response) {
            ...
            if (currentRouteParameters.redirect && currentRouteParameters.statusCode) {
                /* ...and if is it, redirect... */
                NA.redirect(currentRouteParameters, request, response);
            } else {
                /* ...else execute render... */
