...
exports.simpleWebServer = function () {
    ...
    if (NA.configuration.browse) {
        open(url.resolve('http://localhost/', ((typeof NA.configuration.browse === 'string') ? NA.configuration.browse : "")));
    }
    ...
    NA.server.listen(httpPort, function () {
        ...
        NA.log(NA.appLabels.running.publicMode.replace(/%([\-a-zA-Z0-9_]+)%/g, function (regex, matches) { return data[matches]; }));
    });
...
exports.atlasServer = function () {
    ...
    NA.server.listen(httpPort, function () {
        ...
        NA.log(NA.appLabels.running.isRunning.replace(/%([\-a-zA-Z0-9_]+)%/g, function (regex, matches) { return data[matches]; }));
    });
    ...
    if (NA.configuration.browse) {
        open(url.resolve(NA.webconfig.urlWithoutFileName, path.join(NA.webconfig.urlRelativeSubPath, ((typeof NA.configuration.browse === 'string') ? NA.configuration.browse : ""))));
    }
};
...
exports.startingHttpServer = function () {
    ...
    if (!NA.configuration.generate && !NA.webconfig.httpSecure) {
         ...
         */
        NA.server = http.createServer(NA.httpServer);
    } else if (!NA.configuration.generate) {
