...
exports.request = function (path, options) {
    ...
    optionsSupport = setSupport(optionsSupport,
...
exports.executeRequest = function (support, objectPath, options, path) {
    var NA = this,
        ...
        response = function (request, response, next) {
            /* ...else execute render... */
            NA.changeVariationsCommon(response.locals, response.routeParameters, request, response, response.viewsPath, response.currentPath, function () {
                next();
            });
        };
    ...
    /* Execute express middleware for each route. */
    getMiddlewares = (
        ...
        require(pathM.join(NA.serverPath, NA.webconfig.middlewaresRelativePath, routeParameters.middlewares)).bind(NA)
    : function() { return []; };
    ...
    if (support.getSupport) {
        NA.express.get(objectPath, prepare, middlewares, redirect, response);
    }

    /** Execute Post Request */
    if (support.postSupport) {
        NA.express.post(objectPath, prepare, middlewares, redirect, response);
    }

    /** Execute Put Request */
    if (support.putSupport) {
        NA.express.put(objectPath, prepare, middlewares, redirect, response);
    }

    /** Execute Delete Request */
    if (support.deleteSupport) {
        NA.express.delete(objectPath, prepare, middlewares, redirect, response);
    }

    /** Execute Options Request */
    if (support.optionsSupport) {
        NA.express.options(objectPath, prepare, middlewares, redirect, function (request, response) {
    ...
    if (pageNotFound) {
        redirect = function (request, response, next) {
            ...
            if (pageNotFound.redirect && pageNotFound.statusCode) {
                /* ...and if is it, redirect... */
                return NA.redirect(pageNotFound, request, response);
            }
            next();
        };
        prepare = function (request, response, next) {
            /* ...else execute render... */
            NA.prepareResponse(pageNotFoundUrl, NA.webconfig.routes, request, response, function () {
                next();
            });
        };
        response = function (request, response, next) {
        ...
        getMiddlewares = (
            pageNotFound.middlewares) ? 
            require(pathM.join(NA.serverPath, NA.webconfig.middlewaresRelativePath, pageNotFound.middlewares)).bind(NA)
        : function() { return []; };

        middlewares = getMiddlewares();

        /* Match all Get Request */
        NA.express.all("*", prepare, middlewares, redirect, response);
    }
};