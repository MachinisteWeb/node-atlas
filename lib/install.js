...
function copyFile(source, target) {
    ...
    traverse.run(function (error) {
        ...
        log('\u001b[36m' + appLabels.downloadAllModules.installShortcutsDone + '\u001b[39m');
    });
    ...
    traverse.on('complete', function () {
        execute('chmod 755 ' + target + 'nodeatlas', function(error) {
            ...
            if (error && error.cmd && error.cmd.indexOf('cmd.exe') !== -1) {
                isError = false;
            }
            if (isError) {
                log('\u001b[33m' + error + '\u001b[39m');
        ...
        log('\u001b[32m' + appLabels.downloadAllModules.installDone + '\u001b[39m');
    });
}
