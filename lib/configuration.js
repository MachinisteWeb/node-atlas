...
exports.lineCommandConfiguration = function () {
    ...
    /* `lang` manually setted value with `NA.config`. */
    if (commander.lang) {
        NA.configuration.lang = commander.lang;
    }

    /* `init` manually setted value with `NA.config`. */
    if (commander.create) {
        NA.configuration.create = commander.create;
    }

    /* `httpSecure` manually setted value with `NA.config`. */
...
exports.initGlobalVar = function () {
     ...
     * NA.websiteController["common.json"].setModules(...);
...
exports.templateEngineConfiguration = function () {
     ...
     * @default « The `NA#serverPath` value with webconfig's `componentsRelativePath` next ».
     */
    NA.variations.pathname = path.join(NA.serverPath, NA.webconfig.componentsRelativePath);
