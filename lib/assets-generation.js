...
exports.urlGeneratingAssets = function () {
    ...
    async.parallel([
        ...
        if (sourcePath !== destinationPath && htmlGenerationEnable && fs.existsSync(destinationPath)) {
    ...
    if (!NA.configuration.generate &&
        ...
        NA.httpServer.get(NA.webconfig.urlRelativeSubPath + "/", function (request, response) {
            ...
            NA.forEach(NA.webconfig.routes, function (page) {
                var currentRouteParameters;

                if (typeof page !== "string") {
                    currentRouteParameters = page;
                } else {
                    currentRouteParameters = NA.webconfig.routes[page];
                }

                data.page = page;
                if (currentRouteParameters.url) {
                    data.page = currentRouteParameters.url;
                } 
